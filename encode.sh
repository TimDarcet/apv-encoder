#!/bin/bash
 ...
 do
     ...
     if [ -f "$folder_to_encode/constant_quality_output/$foldername/$filename" ]; then
         ...
         ssh -oStrictHostKeyChecking=no $(head -n1 ./computers_name.tmp) "$(dirname $(pwd))/two_pass_one_file.sh $(dirname $(pwd))/${video#../} $folder_to_encode/encoding_final_output/$foldername/$filename $video_bitrate $audio_bitrate" &
